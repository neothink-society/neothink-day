# Day-Of SMS Messages

> **Note:** These messages are copy-pasted directly into GHL workflow SMS actions.
> GHL handles STOP/opt-out compliance automatically — do not add STOP language.

---

## DAY 1 COUNTDOWN

---

### Day 1 — 2 Hours Before
**Offset:** -2h (6:00 AM Pacific / 9:00 AM Eastern)

```
Neothink Society: 2 hours until Day of Enlightenment.

Final prep:
- Workbook ready
- Water nearby
- Quiet space secured

Zoom link coming in 1 hour.

This is the day you've been preparing for.
```

**Character Count:** ~158/160

---

### Day 1 — Zoom Link
**Offset:** -1h (7:00 AM Pacific / 10:00 AM Eastern)

```
Neothink Society: YOUR ZOOM LINK

Doors open: 7:30am PT / 10:30am ET
We begin: 8:00am PT / 11:00am ET SHARP

JOIN: {{trigger_link.bw3pFLlMkAQiQOlYB674}}

ID: {{custom_values.doe_zoom_id}}
Pass: {{custom_values.doe_zoom_passcode}}

Join early. Get settled.
```

**Character Count:** ~160/160 (varies with custom values)

---

### Day 1 — Doors Open
**Offset:** -30m (7:30 AM Pacific / 10:30 AM Eastern)

```
Neothink Society: Doors are open.

Already in the room? Welcome. You're in the right place.

Not in yet? Join us:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

30 minutes until we begin.

The energy is building.
```

**Character Count:** ~155/160

---

### Day 1 — Live
**Offset:** T=0 (8:00 AM Pacific / 11:00 AM Eastern)

```
Neothink Society: We're live.

In the room? Stay present. This is it.

Not in yet? Join us:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

The opening has begun.

Every moment from here forward matters.
```

**Character Count:** ~145/160

---

### Day 1 — Late Joiner (15min)
**Offset:** +15m (8:15 AM Pacific / 11:15 AM Eastern)

```
Neothink Society: We've started.

The opening is happening now. Join:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

You can still catch up. We're saving your seat.
```

**Character Count:** ~120/160

---

### Day 1 — Late Joiner (30min)
**Offset:** +30m (8:30 AM Pacific / 11:30 AM Eastern)

```
Neothink Society: We're underway.

Day of Enlightenment has started. Join us:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

Better late than never. We're here for you.
```

**Character Count:** ~125/160

---

### Day 1 — End of Day
**Offset:** +9h (5:00 PM Pacific / 8:00 PM Eastern)

```
Neothink Society: Day 1 complete. You are a Dream Harvester now.

Day 2 starts this morning: Pre-session 8am PT / 11am ET. Wallace at 9am PT / 12pm ET.

Same Zoom link. See you soon.
```

**Character Count:** ~175/160 — **REQUIRES 2 SEGMENTS**

**Alternative (single segment):**
```
Neothink Society: Day 1 complete. You're a Dream Harvester now.

Day 2 this morning: Pre-session 8am PT / 11am ET. Same Zoom link. See you soon.
```

**Character Count:** ~138/160

---

## DAY 2 COUNTDOWN

---

### Day 2 — 2 Hours Before
**Offset:** +22h (6:00 AM Pacific / 9:00 AM Eastern)

```
Neothink Society: Day 2 in 2 hours.

Pre-session: 8am PT / 11am ET
Wallace takes stage: 9am PT / 12pm ET

Have ready:
- Workbook + Day 1 notes
- Water, pen, paper

Same Zoom link. Today is about Wealth and Integration.
```

**Character Count:** ~195/160 — **REQUIRES 2 SEGMENTS**

**Alternative (single segment):**
```
Neothink Society: Day 2 in 2 hours. Pre-session 8am PT / 11am ET.

Workbook + Day 1 notes ready. Same Zoom link. Today: Wealth & Integration.
```

**Character Count:** ~145/160

---

### Day 2 — 1 Hour Before
**Offset:** +23h (7:00 AM Pacific / 10:00 AM Eastern)

```
Neothink Society: Day 2 pre-session in 1 hour.

8am PT / 11am ET — Pre-session opens
9am PT / 12pm ET — Wallace takes the stage

Gather your Day 1 notes. Same Zoom link. See you soon.
```

**Character Count:** ~150/160

---

### Day 2 — Pre-Session Starting
**Offset:** +24h (8:00 AM Pacific / 11:00 AM Eastern)

```
Neothink Society: Day 2 pre-session starting now.

Join us:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

Wallace takes the stage in 1 hour. Today: Wealth Creation & Level 2 initiation.
```

**Character Count:** ~155/160

---

### Day 2 — 45 Minutes Until Wallace
**Offset:** +24h 15m (8:15 AM Pacific / 11:15 AM Eastern)

```
Neothink Society: 45 minutes until Wallace takes the stage.

In the pre-session? Great. Stay with us.

Joining now?
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

Today we build on yesterday's foundation.
```

**Character Count:** ~155/160

---

### Day 2 — Wallace Live
**Offset:** +25h (9:00 AM Pacific / 12:00 PM Eastern)

```
Neothink Society: Wallace is live. Day 2 is underway.

In the room? Thank you for showing up again.

Not in yet?
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

Today, everything comes together.
```

**Character Count:** ~145/160

---

### Day 2 — Late Joiner (15min)
**Offset:** +25h 15m (9:15 AM Pacific / 12:15 PM Eastern)

```
Neothink Society: Day 2 is happening now.

Integration has started. Join:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

You can still catch up.
```

**Character Count:** ~110/160

---

### Day 2 — Late Joiner (30min)
**Offset:** +25h 30m (9:30 AM Pacific / 12:30 PM Eastern)

```
Neothink Society: Integration is happening.

Day 2 is underway. Join now:
{{trigger_link.bw3pFLlMkAQiQOlYB674}}

Don't miss the completion of your transformation.
```

**Character Count:** ~130/160

---

### Day 2 — Workshop Complete
**Offset:** +33h (5:00 PM Pacific / 8:00 PM Eastern)

```
Neothink Society: You did it.

Two days. Full presence. Real transformation.

You're not the same person you were Friday.

This isn't the end — it's the beginning.

Watch your inbox for next steps.

Welcome to Neothink Society.
```

**Character Count:** ~190/160 — **REQUIRES 2 SEGMENTS**

**Alternative (single segment):**
```
Neothink Society: You did it. Two days. Real transformation.

You're not the same person you were Friday.

Watch your inbox for next steps.

Welcome to Neothink Society. Your golden life has begun.
```

**Character Count:** ~160/160

---

## COMPLETE SEND SCHEDULE

| Message | Offset | Time (PT) | Time (ET) |
|---------|--------|-----------|-----------|
| Day 1 — 2 Hours | -2h | 6:00 AM | 9:00 AM |
| Day 1 — Zoom Link | -1h | 7:00 AM | 10:00 AM |
| Day 1 — Doors Open | -30m | 7:30 AM | 10:30 AM |
| Day 1 — Live | T=0 | 8:00 AM | 11:00 AM |
| Day 1 — Late 15min | +15m | 8:15 AM | 11:15 AM |
| Day 1 — Late 30min | +30m | 8:30 AM | 11:30 AM |
| Day 1 — End of Day | +9h | 5:00 PM | 8:00 PM |
| Day 2 — 2 Hours | +22h | 6:00 AM | 9:00 AM |
| Day 2 — 1 Hour | +23h | 7:00 AM | 10:00 AM |
| Day 2 — Pre-Session | +24h | 8:00 AM | 11:00 AM |
| Day 2 — 45 Minutes | +24h 15m | 8:15 AM | 11:15 AM |
| Day 2 — Live | +25h | 9:00 AM | 12:00 PM |
| Day 2 — Late 15min | +25h 15m | 9:15 AM | 12:15 PM |
| Day 2 — Late 30min | +25h 30m | 9:30 AM | 12:30 PM |
| Day 2 — Complete | +33h | 5:00 PM | 8:00 PM |

---

## SMS BEST PRACTICES

- **No emojis** — Reduces limit from 160 to 70 characters
- **No ALL CAPS** — Except 1-2 words for emphasis
- **No URL shorteners** — Use full URLs or trigger links
- **Business name first** — "Neothink Society:" for identification
- **Single segment preferred** — Under 160 characters when possible
- **GHL handles opt-out** — Do not add STOP language

---

## NOTES

- Uses `{{trigger_link.bw3pFLlMkAQiQOlYB674}}` for tracked Zoom links
- Uses `{{custom_values.doe_zoom_id}}` and `{{custom_values.doe_zoom_passcode}}` for credentials
- Late joiner messages are optional — can skip if using Zoom click tracking
- End-of-day and complete messages have single-segment alternatives provided
